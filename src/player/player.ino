...
class Facade {
  ...
  public:
    ...
    Facade():
      rotary(ROT_SW, HIGH),
      buttons( {
      PushButton(TRIG2, LOW), PushButton(TRIG3, LOW), PushButton(TRIG4, LOW), PushButton(TRIG5, LOW)
    }),
    last_time(0),
...
class FileBrowser {
    ...
    static bool NextPlayableFile(SdFile* dir, SdFile* result, uint16_t* position) {
      do {
        ...
        *position = (dir->curPosition() >> 5) - 1;
        if (result->isFile() && IsPlayable(result)) {
          return true;
        }
      } while (true);
    }
  ...
  public:
    ...
    void ListAlbums() {
      ...
      for (nb_album = 0 ; nb_album < sizeof(albums) / sizeof(albums[0]) ; nb_album++) {
        if (!NextDir(&genre_dir, &album_dir, &albums[nb_album])) {
          break;
        }
      }
      genre_dir.close();
    ...
    void ListTracks( ) {
      ...
      for (nb_track = 0 ; nb_track < sizeof(tracks) / sizeof(tracks[0]) ; nb_track++) {
        if (!NextPlayableFile(&album_dir, &track_file, &tracks[nb_track])) {
          break;
        }
      }
      album_dir.close();
      if (track_file.isOpen()) {
        album_dir.close();
      }
    }
